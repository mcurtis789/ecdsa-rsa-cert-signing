...
def create_ec_cert(cert_authority, private_key,csrin):
    ...
    with open("phone_cert.pem", "wb") as f:
...
def create_rsa_cert(cert_authority, private_key):
    one_day = datetime.timedelta(1, 0, 0)
    # Use our private key to generate a public key
    root_key = serialization.load_pem_private_key(
        private_key, password=b'password'
    )
...
#ca_pem_key = readdata('ca-rsa.key')
#ca_pem_cert = readdata('ca-rsa.crt')
